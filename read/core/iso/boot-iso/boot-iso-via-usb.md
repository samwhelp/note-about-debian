...
``` sh
wget -c https://cdimage.debian.org/debian-cd/13.4.0-live/amd64/iso-hybrid/debian-live-13.4.0-amd64-xfce.iso
```

就會下載一個檔案「debian-live-13.4.0-amd64-xfce.iso」。
...
``` sh
sudo dd if=./debian-live-13.4.0-amd64-xfce.iso of=/dev/sdc bs=4M status=progress && sync
```

或是執行下面指令，就會寫入「USB隨身碟」。

``` sh
sudo dd if=./debian-live-13.4.0-amd64-xfce.iso of=/dev/sdc bs=4M status=progress conv=fsync oflag=direct
```
